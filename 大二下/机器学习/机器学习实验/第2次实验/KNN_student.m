...
% 遍历测试数据集
for i = 1:m
    distances = sqrt(sum((X - Xt(i,:)).^2, 2));  % 计算测试数据点与所有训练数据点的距离
    [~, indices] = sort(distances);  % 按距离从小到大排序，并获取索引,而~表示忽略排序后的距离数组。
    k_nearest_labels = Y(indices(1:K));  % 获取最近的K个样本点的类别标签
    Ym(i) = mode(k_nearest_labels);  % 投票确定预测类别，使用众数作为预测结果
    
    if Ym(i) ~= Yt(i)  % 如果预测类别与真实类别不匹配
        error_count = error_count + 1;  % 错误计数+1
    end
end

error_rate = error_count / m;  % 计算错误率
