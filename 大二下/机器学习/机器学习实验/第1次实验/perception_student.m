...
eta = 0.1;                  % 学习率η
error_count=zeros(max_iter,1);%每次迭代误分类点个数
for i = 1:max_iter
    for j = 1:2*n
        if Y(j) * (dot(X(j,:), w) + b) <= 0
            w = w + eta * Y(j) * X(j,:)';
            b = b + eta * Y(j);
            error_count(i,1)=error_count(i,1)+1;
        end
    end
    if error_count(i,1)==0
        break;
    end
end
...
predictions = zeros(2*m, 1);
for i = 1:2*m
    predictions(i) = sign(dot(Xt(i, :), w) + b);
end

errors = sum(predictions ~= Yt);
error_rate = errors / (2*m);
